...
# === Абстрактний базовий клас для представлення ===
class UserView(ABC):
    @abstractmethod
    def show_contact_card(self, record):
        pass
    ...
    @abstractmethod
    def show_commands_info(self, commands):
        pass
...
class ConsoleView(UserView):
    def show_contact_card(self, record):
        ...
        print(f"Phones: {phones if phones else 'No phones'}")
        print(f"Birthday: {record.birthday.date if record.birthday else 'Not set'}")
        print("--------------------")

    def show_all_contacts(self, records):
        print("=== All Contacts ===")
        for record in records:
            self.show_contact_card(record)
...
def main():
    ...
    while True:
        ...
        if command in ["close", "exit"]:
            ...
            view.show_commands_info(COMMANDS)
            ...
            records = show_all(book)
            view.show_all_contacts(records)
